...
function random6DigitCode(): string {
  // Generate exactly 6 digits (000000 - 999999), leading zeros allowed
...
function normalizeInviteDoc(docId: string, data: Record<string, unknown>): InviteCode {
  return {
    ...
    academyId: (data?.academyId as string) ?? (data?.schoolId as string) ?? "",
...
type InviteCodeState = {
  ...
  getCodesByAcademy: (academyId: string) => InviteCode[];
  generateCode: (academyId: string, role: InviteCodeRole, userId: string) => Promise<string>;
...
export const useInviteCodeStore = create<InviteCodeState>((set, get) => ({
  ...
  loadCodes: async (schoolId: string) => {
    if (!schoolId) {
      console.warn("[InviteCodes] loadCodes called without schoolId");
      return;
    }
    set({ isLoading: true });
    try {
      ...
      // Query by canonical schoolId field (new docs)
      const qNew = query(
      ...
      const newDocs = snapNew.docs.map((d) => normalizeInviteDoc(d.id, d.data() as Record<string, unknown>));

      // Fallback: also query legacy docs that only have academyId (no schoolId)
      const qLegacy = query(
        collection(db, INVITES_COL),
        where("academyId", "==", schoolId),
        where("active", "==", true)
      );
      const snapLegacy = await getDocs(qLegacy);
      const legacyDocs = snapLegacy.docs
        .map((d) => normalizeInviteDoc(d.id, d.data() as Record<string, unknown>))
        // Exclude any that were already returned by the first query
        .filter((d) => !newDocs.some((n) => n.id === d.id));

      const codes = [...newDocs, ...legacyDocs];
      set({ codes });
    } catch (e) {
  ...
  getCodesByAcademy: (academyId: string) => {
    if (!academyId) return [];
    return get().codes.filter((c) => c.academyId === academyId);
  },

  generateCode: async (academyId: string, role: InviteCodeRole, userId: string) => {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      try {
        await runTransaction(db, async (tx) => {
          ...
          tx.set(ref, {
            code,
            schoolId: academyId,   // canonical field used by rules/join
            academyId,             // kept for backward compat with legacy docs
            role,
            active: true,
            usedCount: 0,
            createdBy: userId,
  ...
  toggleCodeActive: async (codeId) => {
    ...
    set((s) => ({
      codes: s.codes.map((c) => (c.id === codeId ? { ...c, active: !c.active } : c)),
    }));
  },
  ...
  deleteCode: async (codeId) => {
    ...
    set((s) => ({
      codes: s.codes.filter((c) => c.id !== codeId),
    }));
  },
}));

function InviteCodeCard({ invite, onToggle, onDelete, onCopy }: {
  invite: InviteCode;
  onToggle: () => void;
  onDelete: () => void;
  onCopy: () => void;
      ...
      >
        <View className="flex-row items-center justify-between mb-2">
          <View className={`px-2 py-1 rounded-full ${invite.active ? "bg-green-100" : "bg-gray-100"}`}>
            <Text
              className={`text-xs font-medium ${invite.active ? "text-green-700" : "text-gray-500"}`}
          ...
          <View className={`px-2 py-1 rounded-full ${invite.role === "coach" ? "bg-blue-100" : "bg-purple-100"}`}>
            <Text
              className={`text-xs font-medium ${invite.role === "coach" ? "text-blue-700" : "text-purple-700"}`}
              style={{ fontFamily: "Outfit_500Medium" }}
            >
        ...
        <Text className="text-2xl font-bold text-gray-900 tracking-wider text-center my-3" style={{ fontFamily: "Poppins_700Bold" }}>
          {invite.code}
        </Text>

        <View className="flex-row items-center justify-center mb-3">
          <Users size={14} color="#6B7280" />
          <Text className="text-gray-500 text-sm ml-1" style={{ fontFamily: "Outfit_400Regular" }}>
        ...
        <View className="flex-row pt-3 border-t border-gray-100">
          ...
          >
            ...
            <Text className="text-gray-600 text-sm ml-1" style={{ fontFamily: "Outfit_500Medium" }}>
              Copy
            </Text>
          </Pressable>
          ...
          >
            ...
            <Text className="text-red-600 text-sm ml-1" style={{ fontFamily: "Outfit_500Medium" }}>
              Delete
            </Text>
          </Pressable>
        </View>
...
export function InviteCodesSection() {
  ...
  const schoolCodes = codes.filter((c) => c.academyId === schoolId);
  ...
  const handleGenerate = async (role: InviteCodeRole) => {
    ...
    try {
      ...
      // Reload list after creation to ensure consistency
  ...
  return (
    <ScrollView className="flex-1 px-4 pt-4" showsVerticalScrollIndicator={false}>
      <Text className="text-lg font-bold text-gray-800 mb-3" style={{ fontFamily: "Poppins_600SemiBold" }}>
        Generate New Codes
      </Text>
      ...
      <View className="flex-row mb-6">
        ...
        >
          ...
          <Text className="text-white font-medium ml-2" style={{ fontFamily: "Outfit_500Medium" }}>
            Student Code
          </Text>
        </Pressable>
      ...
      <Text className="text-lg font-bold text-gray-800 mb-3" style={{ fontFamily: "Poppins_600SemiBold" }}>
        Active Codes ({schoolCodes.length})
      </Text>

      {schoolCodes.length === 0 ? (
        <View className="items-center py-12 bg-gray-50 rounded-2xl">
          <Users size={48} color="#9CA3AF" />
          <Text className="text-gray-400 mt-4 text-center" style={{ fontFamily: "Outfit_400Regular" }}>
